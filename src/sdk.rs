...
impl VectorKernel {
    ...
    pub fn transfer(&mut self, from_id: &str, to_id: &str, amount: Vec<u128>) -> Result<(VectorStateV1, VectorStateV1), KernelXError> {
    ...
    pub fn project(&mut self, vector_id: &str, projected_components: Vec<u128>, escrow_id: impl Into<String>) -> Result<VectorStateV1, KernelXError> {
        self.engine.project(vector_id, projected_components, escrow_id)
    }

    pub fn reconstruct(&mut self, vector_id: &str, outcome: SettlementOutcome) -> Result<VectorStateV1, KernelXError> {
